...
async function doTask() {
    ...
    //然后获取要绑定的手机号
    let mobile = await httpGet(bindMobileUrl);
    if (!mobile) {
        logger.info("no_get_mobile");
    ...
    try {
        ...
        logger.info('get-bind-mobile=' + mobile);
        addLog(mobile, 'get-bind-mobile');
        ...
        if (currentUrl.includes('exchange')) {
            ...
            logger.info("changYouScore=" + changYouScore + ",mobile=" + mobile);
        ...
        logger.error("do_task_error,mobile=" + mobile, e);
    } finally {
        if (page) {
            try {
                ...
                logger.error("close_page_error,mobile=" + mobile, e);
            }
        }
        logger.info(browserItem.userDataDir + " disconnect,mobile=" + mobile);
        // 断开浏览器连接
        if (browser) {
...
async function doExchange(page, mobile, changYouScore) {
    ...
    if (needsChangYouScore === '-1') {
        await addLog(mobile, "no_enough_score=" + ydScore);
        return;
    } else if (needsChangYouScore === '0') {
        await addLog(mobile, "has_enough_chang_you_score=" + changYouScore);
        //绑定成功，更新状态到数据库  3已兑换
...
async function getExchangeScore(mobile, dyScore, hasChangYouScore) {
    ...
    logger.info('exchangeScoreUrl=' + url);
